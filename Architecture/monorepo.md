...
- **Justify the Submodule:** Only use a submodule if the code truly needs to live independently. For example, if it's a shared utility library used by external projects *outside* the monorepo. If it's only ever used inside this monorepo, it's often better to just merge the code directly into the monorepo tree.
...
- **Always Clone Recursively:** Train team members to clone the monorepo with the recurse flag:
  ...
  ```
  *Tip: If a developer accidentally clones without this flag, they can initialize and fetch the missing submodules by running:*
  ```bash
...
- **Bottom-Up Commits:** Always commit and push changes in the submodule *first*, then commit the updated reference pointer in the root monorepo.
...
- [Example Repository](https://github.com/newelectronicuser/monorepo/tree/master)
